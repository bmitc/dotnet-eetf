...
let ``Encode atoms`` () =
    ...
    for atom in testAtoms do
        ...
        |> encodeTermToBytes
        |> decodeTermFromBytes
        |> should equal (ErlangTerm.Atom atom)

[<Fact>]
let ``Encode small integers`` () =
    let testIntegers = [0; 1; 99; 101; 254; 255]
    for integer in testIntegers do
        ErlangTerm.Integer integer
        |> encodeTermToBytes
        |> decodeTermFromBytes
        |> should equal (ErlangTerm.Integer integer)
...
let ``Encode small big integers`` () =
    ...
    for bigInteger in testBigIntegers do
        ErlangTerm.BigInteger bigInteger
        |> encodeTermToBytes
        |> decodeTermFromBytes
        |> should equal (ErlangTerm.BigInteger bigInteger)
